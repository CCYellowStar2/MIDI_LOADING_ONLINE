...
class AutoPianoPlayer:
    ...
    def play(self, context: Context, settings: AutoPianoSettings) -> int:
        ...
        played = self.play_notes(context, notes, bridge, settings.speed, settings.transpose)
        print(f"AutoPiano finished. {played} notes sent through Maa controller.", flush=True)
        return played
    ...
    @classmethod
    def sleep_interruptibly(cls, context: Context, seconds: float, step: float = 0.02) -> None:
    ...
    def play_notes(
        ...
        while index < len(notes):
            ...
            chord, next_index = cls.collect_chord(notes, index, transpose, DEFAULT_CHORD_WINDOW)
